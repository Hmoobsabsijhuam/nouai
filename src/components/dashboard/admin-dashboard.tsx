...
import { collection, Timestamp, query, orderBy, limit, updateDoc, doc, serverTimestamp } from 'firebase/firestore';
import { useCollection, WithId } from '@/firebase/firestore/use-collection';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@/components/ui/dialog';
import { Badge } from '@/components/ui/badge';
import { Users, Send, CalendarIcon, Clock, LifeBuoy, Bell, CreditCard } from 'lucide-react';
...
function TicketDetailsDialog({ ticket, onOpenChange, onStatusChange }: { ticket: WithId<SupportTicket> | null; onOpenChange: (open: boolean) => void; onStatusChange: (ticketId: string, status: 'open' | 'closed') => void; }) {
  return (
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            <Select value={ticket?.status} onValueChange={(value: 'open' | 'closed') => ticket && onStatusChange(ticket.id, value)}>
...
export default function AdminDashboard({ user }: { user: any }) {
  ...
  // Simplified users query to fetch all users
  ...
  // Use admin_notifications as the source for purchase history
  ...
  const { userChartData, totalUsers } = useMemo(() => {
    ...
    return { userChartData: chartData, totalUsers: users.length };
  }, [users]);
  ...
  const { creditChartData, monthlySummary } = useMemo(() => {
    ...
    const monthlySummary = purchases.filter(n => isWithinInterval(toSafeDate(n.createdAt), { start: monthStart, end: monthEnd }))
      .reduce((acc, notif) => {
        const creditAmountMatch = notif.message.match(/(\d+)\s*credits/);
        const credits = creditAmountMatch ? parseInt(creditAmountMatch[1], 10) : 0;
        const priceMatch = notif.message.match(/\$(\d+)/);
    ...
    return { creditChartData: chartData, monthlySummary };
  }, [adminNotifications]);

  const handleBarClick = (data: UserChartDataItem) => setSelectedDay(data);
  ...
  const handleTicketStatusChange = async (ticketId: string, status: 'open' | 'closed') => {
    ...
    try {
      ...
      toast({ title: 'Ticket Updated', description: `Ticket status changed to ${status}.` });
  ...
  const recentPurchases = useMemo(() => {
      return adminNotifications?.filter(n => n.paymentStatus === 'paid').slice(0, 5) ?? [];
  }, [adminNotifications]);


  return (
    <div>
      <h1 className="mb-4 text-3xl font-bold tracking-tight">Admin Control Panel</h1>
      ...
      <div className="mb-6 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        <Card>
          ...
          <CardContent>
            {isUsersLoading ? <Skeleton className="h-8 w-1/4" /> : <div className="text-2xl font-bold">{totalUsers}</div>}
            <p className="text-xs text-muted-foreground mb-4">Total number of users in the system.</p>
            <Button size="sm" onClick={() => setIsUserListOpen(true)} disabled={isUsersLoading}>View All Users</Button>
          </CardContent>
        </Card>
        <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
                <p className="text-xs text-muted-foreground mb-4">Broadcast a message to all users.</p>
                <Button asChild size="sm"><Link href="/notifications"><Send className="mr-2 h-4 w-4" /> Go to Notifications</Link></Button>
            </CardContent>
        </Card>
         <Card>
           <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            {isTicketsLoading ? <Skeleton className="h-8 w-1/4" /> : <div className="text-2xl font-bold">{openSupportTickets.length}</div>}
            <p className="text-xs text-muted-foreground">New issues reported by users.</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 lg:gap-8">
          <div className="lg:col-span-2 space-y-6">
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><CalendarIcon className="h-5 w-5" />Daily User Registrations</CardTitle>
                    <CardDescription>Click on a bar to see the users registered on that day.</CardDescription>
                </CardHeader>
                <CardContent>
                {isUsersLoading ? <Skeleton className="h-[300px] w-full" /> : userChartData.length > 0 ? (
                    <ResponsiveContainer width="100%" height={300}>
                        <BarChart data={userChartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }} onClick={(e) => e && e.activePayload && handleBarClick(e.activePayload[0].payload)}>
                            ...
                            />
                            <Bar dataKey="count" name="New Users">
                                {userChartData.map((_, index) => <Cell key={`cell-${index}`} fill="hsl(var(--primary))" className="cursor-pointer" />)}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                ) : (
                    <div className="flex h-[300px] items-center justify-center"><p className="text-muted-foreground">No user registration data yet.</p></div>
                )}
                </CardContent>
            </Card>
            <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><CreditCard className="h-5 w-5" />Daily Credit Purchases</CardTitle>
                    <CardDescription>Total credits sold and revenue generated per day.</CardDescription>
                </CardHeader>
                <CardContent>
                    {isAdminNotifsLoading ? <Skeleton className="h-[300px] w-full" /> : creditChartData.length > 0 ? (
                        <>
                            <ResponsiveContainer width="100%" height={300}>
                                <BarChart data={creditChartData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                                        ...
                                        content={({ active, payload, label }) => {
                                            if (active && payload && payload.length) {
                                                return (
                                                    <div className="rounded-lg border bg-background p-2 shadow-sm">
                                                        <p className="font-bold">{`${format(new Date(label), 'eeee, MMM d')}`}</p>
                                                        <p className="text-sm text-primary">Credits: {payload[0].value}</p>
                                                        <p className="text-sm text-primary">Revenue: ${payload[1].value?.toFixed(2)}</p>
                                                    </div>
                                                );
                                            }
                                            return null;
                                    ...
                                    />
                                    <Bar dataKey="credits" fill="hsl(var(--primary))" name="Credits Sold" />
                         ...
                         <div className="flex h-[300px] items-center justify-center"><p className="text-muted-foreground">No credit purchase data yet.</p></div>
                    )}
                </CardContent>
            </Card>
          </div>
        ...
        <div className="space-y-6 lg:space-y-8">
            <Card>
             ...
             <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><CreditCard className="h-5 w-5" />Credit Purchase Alerts</CardTitle>
                    <CardDescription>Recent credit refills by users.</CardDescription>
                </CardHeader>
                <CardContent>
                    {isAdminNotifsLoading ? <Skeleton className="h-24 w-full" /> : recentPurchases.length > 0 ? (
                        <ScrollArea className="h-48">
                            <div className="space-y-2 pr-4">
                                {recentPurchases.map(notif => (
             ...
             <Card>
                <CardHeader>
                    <CardTitle className="flex items-center gap-2"><LifeBuoy className="h-5 w-5" />Open Support Tickets</CardTitle>
      ...
      <UserListDialog isOpen={isUserListOpen} onOpenChange={setIsUserListOpen} users={users} isLoading={isUsersLoading} />
      <TicketDetailsDialog ticket={selectedTicket} onOpenChange={() => setSelectedTicket(null)} onStatusChange={handleTicketStatusChange} />
    </div>
  );
}
